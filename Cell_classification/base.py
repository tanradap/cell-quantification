...
def find_hema_to_remove_slide(hema_input):
    """
    Find cells with hematoxylin above criteria, put in a list needed to be removed.
    """
    h2 = hema_input.copy()
    hema = hema_input['Hematoxylin: Nucleus: Mean']
    # normalise
    threshold = hema.quantile(0.99)
    hema_norm = hema/threshold
    # put in new variable
    h2.loc[:, 'Hematoxylin: Nucleus: Mean'] = hema_norm
    hema_to_remove = h2[h2['Hematoxylin: Nucleus: Mean'] > 1]  # to select instances need removing (keep <=1)

    return hema_to_remove


def remove_cell_hema_slide(cell_nb_input,
                           hema_to_remove):
    """
    Remove cells with hema that fit the criteria, return
    cleaned outputs & removal log if it was successful or not.
    """
    i = cell_nb_input  # annotated cells with NNB
    h = hema_to_remove  # cells we need to remove, may or may not contain annotated cells

    # Find cells that exist in both 'i' & 'h' = cells we want to remove
    to_remove = i.merge(h,
                        on=['Centroid_X', 'Centroid_Y'],
                        how='inner',
                        validate='1:1') # shape will change
    ...
    to_retain = i.merge(h,
                        ...
                        how='left',
                        indicator=True,
                        validate='1:1') # shape remains same as i

    # Extract cells we want to retain
    retained = i[to_retain['_merge'] == 'left_only']

    # Extract cells we want to remove
    removed = i[to_retain['_merge'] == 'both']

    # Boolean to say if operation was successful or not
    removed_log = (to_remove.shape[0] == (removed.shape[0]))

    return retained, removed, removed_log
