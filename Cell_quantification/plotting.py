...
def subset_psp_stage(df, stage):
    ...
    for s in stages:
        stage_s = df[df[stage] == s]
        stage_s_subset = stage_s[['Image_name',
                                  'region_name',
                                  'd_hallmark',
                                  'd_Neuron',
                                  'd_Astro',
...
def polar_plot_log(dict_subset,
               dict_mean,
               tau_type,
               val_add,
               anatomical_order,
    ...
    min_val = min(np.log10(dict_subset[min_stage][tau_type]))
...
def polar_plot(dict_subset,
    ...
    # set plotting value range
    min_val = min((dict_subset[min_stage][tau_type]))
    max_val = max((dict_subset[max_stage][tau_type] + val_add))
    print('min value: ', min_val)
    print('max value: ', max_val)
    ...
    fig.update_layout(
                    ...
                    title_text=title,
                    polar=dict(
                        radialaxis=dict(tickangle=tickangle_val,
                                        range=[min_val, max_val],
                                        showticklabels=tick_label,
                                        ticks='outside',
                                        dtick=1
